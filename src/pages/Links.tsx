		...
		<div className="h-full w-full p-8 md:p-16 overflow-y-auto custom-scrollbar">
			<div className="max-w-5xl mx-auto">
				<motion.div
					...
					className="mb-12 border-b border-zinc-800 pb-8 flex flex-col md:flex-row justify-between md:items-end gap-4"
				>
					<div>
						<h2 className="text-xs font-mono text-accent mb-2">// {t("links.subtitle")}</h2>
						<h1 className="text-6xl font-sans font-bold text-white">{t("links.title")}</h1>
					</div>
					...
					{links.map((link, index) => {
						...
						return (
							<motion.a
								...
								whileTap={{ scale: 0.98 }}
								className="group relative bg-surface border border-zinc-800 h-48 p-8 flex flex-col justify-between overflow-hidden hover:border-accent transition-colors duration-300"
							>
								...
								<div className="relative z-10">
									...
									<h3 className="text-3xl font-bold font-sans text-white group-hover:text-black transition-colors">
										{link.label}
									</h3>
								</div>

								<div className="absolute top-0 right-0 w-8 h-8 border-l border-b border-zinc-800 bg-bg z-10 group-hover:bg-accent group-hover:border-black/10 transition-colors" />
							</motion.a>
						);
					})}
				</div>
